...
variable "alert_configuration" {
  description = "Alert configuration blocks. status/notification_medium are provider-native numeric codes."
  type = list(object({
    status                 = optional(number)
    business_hours_id      = optional(string)
    notification_medium    = list(number)
    outside_business_hours = optional(bool)
  }))
  nullable = false
  default  = []
}
...
variable "notification_delay_configuration" {
  description = "Notification delay configuration blocks (provider-native numeric codes)."
  type = list(object({
    status                 = optional(number)
    business_hours_id      = optional(string)

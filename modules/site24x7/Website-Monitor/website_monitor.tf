...
resource "site24x7_website_monitor" "website_monitor" {
  lifecycle {
    precondition {
      ...
      error_message = "Set at most one of location_profile_id or location_profile_name, not both."
    }

    precondition {
      condition     = !(var.notification_profile_id != null && var.notification_profile_name != null)
      error_message = "Set at most one of notification_profile_id or notification_profile_name, not both."
    }
  }

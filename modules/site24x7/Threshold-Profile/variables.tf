...
variable "ssl_cert_days_until_expiry_critical_threshold" {
  description = "SSL-days-until-expiry critical threshold object (provider-native map, e.g. { severity, value }). Use severity = 3 for the critical level, matching the site24x7 trouble = 2 / critical = 3 convention."
  type        = any
  default     = null
}
